...
"""Creates a dynamic version of bidirectional recurrent neural network.
  ...
  """
	...
	-- forward rnn
  self.dropout = utils.getopt(opt, 'dropout', 0.8)
...
inputs[2] is sequence length, including the length of each sequence
  ...
  if self.state_type == 'final' then
    ...
    for i = 1, batch_size do
      output_states[i][{{1, outputs[1]:size(2)/2}}]:copy(outputs[sequence_length[i]][i][{{1, outputs[1]:size(2)/2}}])
    end
    output_states[{{}, {outputs[1]:size(2)/2 + 1, -1}}]:copy(outputs[1][{{}, {outputs[1]:size(2)/2 + 1, -1}}])
  else
